import { TranslationType } from './en';

export const te: TranslationType = {
  nav: {
    home: "హోమ్",
    medicines: "మందులు",
    ayurbook: "ఆయుర్వేదం",
    timeline: "టైమ్‌లైన్",
    profile: "ప్రొఫైల్",
    login: "లాగిన్",
    signup: "ఖాతా తెరవండి",
    logout: "లాగ్ అవుట్",
    subtitle: "డిశ్చార్జ్ తరువాతి కోలుకునే తోడ్పాటు"
  },
  home: {
    title: "కేర్ డాష్‌బోర్డ్",
    subtitle: "మీ దైనందిన కోలుకునే స్థితి, మందులు మరియు భద్రతా తనిఖీలు",
    welcome: "తిరిగి స్వాగతం",
    recoveryStatus: "కోలుకునే స్థితి",
    activeCondition: "ప్రస్తుత ఆరోగ్యం",
    todaysMedicines: "నేటి మందుల పట్టిక",
    takeDose: "మందు తీసుకోండి",
    taken: "తీసుకున్నారు",
    skipped: "వదిలేసారు",
    snoozed: "ఆలస్యం చేయబడింది",
    logSymptom: "లక్షణ నమోదు",
    checkAyurbook: "ఆయుర్వేద భద్రతా తనిఖీ",
    recentTimeline: "ఇటీవలి కోలుకునే టైమ్‌లైన్",
    viewAll: "అన్నీ చూడండి",
    noMedicines: "ఇంకా మందులు ఏవీ జోడించబడలేదు."
  },
  medicines: {
    title: "ప్రస్తుత మందులు",
    subtitle: "మీ మందుల వేళలు మరియు జ్ఞాపికలను నిర్వహించండి",
    addMedicine: "మందును జోడించండి",
    scanPrescription: "ప్రిస్క్రిప్షన్ స్కాన్ చేయండి",
    doseSchedule: "మందుల వేళల పట్టిక",
    frequency: "ఫ్రీక్వెన్సీ",
    instructions: "సూచనలు",
    reminderTime: "జ్ఞాపిక సమయం",
    delete: "తొలగించు"
  },
  ayurbook: {
    title: "ఆయుర్వేద లైబ్రరీ",
    subtitle: "సాంప్రదాయ ఆయుర్వేద నివారణలు & మందుల అనుకూలత తనిఖీ",
    searchPlaceholder: "మూలికలు లేదా నివారణల కోసం వెతకండి...",
    checkInteraction: "మందులు & మూలికల కలయిక తనిఖీ",
    safe: "వాడకానికి సురక్షితం",
    caution: "జాగ్రత్త వహించండి",
    avoid: "కలిపి వాడకండి",
    viewRemedy: "వివరాలు చూడండి"
  },
  timeline: {
    title: "కోలుకునే టైమ్‌లైన్",
    subtitle: "మీ ఆరోగ్య ప్రయాణం మరియు మందుల చరిత్ర నమోదు",
    allEvents: "అన్ని ఈవెంట్‌లు",
    medications: "మందులు",
    symptoms: "లక్షణాలు",
    remedies: "నివారణలు",
    today: "ఈ రోజు"
  },
  profile: {
    title: "ప్రొఫైల్ & సెట్టింగ్‌లు",
    subtitle: "మీ ఖాతా, యాప్ భాష, థీమ్ మరియు డేటా సెట్టింగ్‌లను నిర్వహించండి",
    userAccount: "యూజర్ ఖాతా",
    authenticated: "ధృవీకరించబడిన ఖాతా",
    rlsActive: "రో-లెవెల్ సెక్యూరిటీ యాక్టివ్",
    settings: "యాప్ సెట్టింగ్‌లు",
    changeLanguage: "యాప్ భాష",
    selectLanguage: "భాషను ఎంచుకోండి",
    theme: "థీమ్ రూపం",
    lightMode: "లైట్ మోడ్",
    darkMode: "డార్క్ మోడ్",
    dangerZone: "డేంజర్ జోన్",
    deleteAccount: "ఖాతాను తొలగించండి",
    deleteAccountDesc: "మీ ఖాతా మరియు మొత్తం మందుల సమాచారాన్ని శాశ్వతంగా తొలగించండి.",
    patientCarePlan: "పేషెంట్ కేర్ ప్లాన్",
    myMedicines: "నా మందులు",
    myRecovery: "నా రికవరీ",
    privacySecurity: "గోప్యత మరియు డేటా భద్రత"
  },
  auth: {
    loginTitle: "తిరిగి స్వాగతం 👋",
    loginSubtitle: "మీ కేర్ ఖాతాలోకి లాగిన్ అవ్వండి",
    signupTitle: "CareConnect ఖాతాను సృష్టించండి",
    signupSubtitle: "పోస్ట్-డిశ్చార్జ్ రికవరీ ప్లాట్‌ఫారమ్‌లో చేరండి",
    email: "ఈమెయిల్ చిరునామా",
    password: "పాస్‌వర్డ్",
    fullName: "పూర్తి పేరు",
    age: "వయస్సు",
    confirmPassword: "పాస్‌వర్డ్‌ను నిర్ధారించండి",
    continueGoogle: "Googleతో కొనసాగించండి",
    orEmail: "లేదా ఈమెయిల్‌తో",
    noAccount: "ఖాతా లేదా?",
    alreadyAccount: "ఇప్పటికే ఖాతా ఉందా?",
    forgotPassword: "పాస్‌వర్డ్ మర్చిపోయారా?"
  },
  deleteModal: {
    title: "ఖాతాను తొలగించాలా?",
    warning: "ఈ చర్య శాశ్వతమైనది. మీ మందుల వివరాలు మరియు ఆరోగ్య డేటా తొలగించబడతాయి.",
    confirmText: 'నిర్ధారణకు "DELETE" అని టైప్ చేయండి:',
    cancel: "రద్దు చేయి",
    confirmDelete: "ఖాతాను శాశ్వతంగా తొలగించు"
  }
};
