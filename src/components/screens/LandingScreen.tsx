    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-brand-50/20 to-slate-50 pb-20">
      ...
      <section className="relative pt-12 pb-16 px-4 overflow-hidden">
        <div className="max-w-4xl mx-auto text-center relative z-10 space-y-6">
          ...
          {/* Brand Header */}
          <div className="inline-flex items-center gap-2 bg-brand-100/80 border border-brand-200 text-brand-800 text-xs font-bold px-4 py-1.5 rounded-full shadow-xs uppercase tracking-wider">
          ...
          {/* EXACTLY TWO OPTIONS: Create Account & Log In */}
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 max-w-md mx-auto pt-4">
            ...
            <button
              ...
              className="w-full sm:w-1/2 flex items-center justify-center gap-2 bg-white hover:bg-slate-50 text-slate-800 font-extrabold text-sm py-4 px-6 rounded-2xl border border-slate-300 shadow-sm transition"
            >
      ...
      <section className="px-4 py-8">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-6">
            <h2 className="text-xl sm:text-2xl font-bold text-slate-900">CareConnect Recovery Features</h2>
            <p className="text-xs text-slate-500 mt-1">Prescription Manager • Daily Dosage Alerts • AyurBook Remedies • Timeline Log</p>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-center">
            {[
              { title: "1. Home Hub", desc: "Daily dosage alerts & next actions", color: "bg-teal-50 border-teal-200 text-teal-900" },
              { title: "2. Medicines", desc: "Prescription schedule & upload", color: "bg-slate-50 border-slate-200 text-slate-800" },
              { title: "3. AyurBook", desc: "Herbal remedies & interaction check", color: "bg-emerald-50 border-emerald-200 text-emerald-900" },
              { title: "4. Timeline", desc: "Recovery history & dosage logs", color: "bg-indigo-50 border-indigo-200 text-indigo-900" },
            ].map((step, idx) => (
